...
struct deferred_val {
	struct deferred_val *next;
	void *ptr;
};

struct poptrie_snapshot {
	struct rcu_head		 rcu;
	struct deferred_val	 *deferred_free; /* freed after RCU grace */
	u32			 *dir;
...
{
	struct deferred_val *d, *next;

	if (!s)
		return;
	for (d = s->deferred_free; d; d = next) {
		next = d->next;
		kfree(d->ptr);
		kfree(d);
	}
...
static void snap_free_rcu(struct rcu_head *h)
{
	snap_free(container_of(h, struct poptrie_snapshot, rcu));
}
...
/* -------------------------------------------------------------------------
 * build_dir — single recursive radix walk that fills the dir[] tier.
 *
 * Visits each radix node at most once down to depth POPTRIE_S.  When the
 * radix subtree is exhausted (n == NULL) before depth POPTRIE_S, the entire
 * remaining dir[] subrange shares one leaf value (the current ext) and is
 * filled in one tight loop.  This replaces 2^POPTRIE_S independent root-
 * to-depth descents and is the dominant rebuild speedup for sparse tries.
 * ------------------------------------------------------------------------- */
static int build_dir(struct poptrie_snapshot *s, struct radix_node *n,
		     int depth, u32 prefix_idx,
		     const struct radix_node *ext)
{
	int err;
	...
	if (depth == POPTRIE_S) {
		if (radix_has_children(n)) {
			...
			err = build_subtree_at(s, n, POPTRIE_S, ext, node_idx);
			if (err)
				return err;
			s->dir[prefix_idx] = node_idx;
		} else {
			s->dir[prefix_idx] = DIR_LEAF_BIT | eff_fib(n, ext);
		}
		return 0;
	}

	if (!n) {
		u32 leaf = DIR_LEAF_BIT | (ext ? ext->fib_idx : 0);
		u32 span = 1U << (POPTRIE_S - depth);
		u32 i;

		for (i = 0; i < span; i++)
			s->dir[prefix_idx + i] = leaf;
		return 0;
	}

	err = build_dir(s, n->left, depth + 1, prefix_idx, ext);
	if (err)
		return err;
	return build_dir(s, n->right, depth + 1,
			 prefix_idx | (1U << (POPTRIE_S - 1 - depth)), ext);
}
...
static int poptrie_rebuild(struct lpm_trie *trie, struct deferred_val *defer)
{
	...
	/*
	 * Pre-size nodes[] and leaves[] from n_entries to skip ~12 doubling
	 * reallocs (each of which copies all prior data) on large tables.
	 * These are upper-bound heuristics: a poptrie internal node is
	 * created for at most every K bits of any prefix beyond depth
	 * POPTRIE_S, and each prefix contributes at most a small constant
	 * number of leaf slots.  Over-estimating just wastes transient RAM
	 * — under-estimating falls back to snap_grow_* doubling.
	 */
	{
		u32 want_nodes  = n ? (n + 256) : 256;
		u32 want_leaves = n ? (n * 4 + 256) : 256;

		while (s->nodes_cap  < want_nodes) {
			err = snap_grow_nodes(s);
			if (err)
				goto err;
		}
		while (s->leaves_cap < want_leaves) {
			err = snap_grow_leaves(s);
			if (err)
				goto err;
		}
	...
	err = build_dir(s, trie->radix, 0, 0, NULL);
	if (err)
		goto err;
...
install:
	...
	if (old) {
		/* Append defer list to old->deferred_free, then queue free. */
		if (defer) {
			struct deferred_val *tail = defer;

			while (tail->next)
				tail = tail->next;
			tail->next = old->deferred_free;
			old->deferred_free = defer;
		}
		call_rcu(&old->rcu, snap_free_rcu);
	} else {
		/* No prior snapshot — nothing can reference these values. */
		struct deferred_val *d, *next;

		for (d = defer; d; d = next) {
			next = d->next;
			kfree(d->ptr);
			kfree(d);
		}
...
/* Wrap a single user value into a deferred_val list element. */
static struct deferred_val *defer_wrap(void *ptr)
{
	struct deferred_val *d;

	if (!ptr)
		return NULL;
	d = kmalloc(sizeof(*d), GFP_KERNEL | __GFP_NOWARN);
	if (!d) {
		/* Last-resort: leak rather than UAF.  Practically unreachable. */
		return NULL;
	}
	d->ptr = ptr;
	d->next = NULL;
...
/*
 * Core radix mutation for an update.  Caller holds update_mutex and is
 * responsible for triggering poptrie_rebuild() afterwards.  On success,
 * *old_val_out receives any displaced caller value (NULL if none) so the
 * caller can hand it to poptrie_rebuild() for deferred RCU free.
 */
static int __trie_update_locked(struct lpm_trie *trie, u32 addr, u32 plen,
				void *val, u64 flags, void **old_val_out)
{
	void *old_val = NULL;
	int ret;
	...
	*old_val_out = NULL;
	...
	ret = radix_insert(&trie->radix, addr, (int)plen, val, 0, NULL);
	...
	if (trie->n_entries == trie->map.max_entries) {
		/* Undo: caller still owns val and will kfree it on error. */
		radix_delete(&trie->radix, addr, (int)plen, 0, NULL, &old_val);
		return -ENOSPC;
	}
	trie->n_entries++;
	return 0;
			     ...
			     void *_key, void *value, u64 flags)
{
	...
	ret = __trie_update_locked(trie, addr, plen, val, flags, &old_val);
	if (ret) {
		kfree(val);
		mutex_unlock(&trie->update_mutex);
		return ret;
	}
	ret = poptrie_rebuild(trie, defer_wrap(old_val));
	mutex_unlock(&trie->update_mutex);
	return ret;
}

static int __trie_delete_locked(struct lpm_trie *trie, u32 addr, u32 plen,
				void **old_val_out)
{
	int ret = radix_delete(&trie->radix, addr, (int)plen, 0, NULL,
			       old_val_out);
	if (ret)
		return ret;
	trie->n_entries--;
...
static long trie_delete_elem(struct bpf_map *map, void *_key)
{
	...
	mutex_lock(&trie->update_mutex);
	ret = __trie_delete_locked(trie, addr, plen, &old_val);
	if (ret) {
		mutex_unlock(&trie->update_mutex);
		return ret;
	}
...
/* -------------------------------------------------------------------------
 * Batch ops — the dominant optimization for bulk loads (BGP tables).
 *
 * Generic batch ops loop calling map_update_elem / map_delete_elem, which
 * triggers a full poptrie rebuild per element — O(N^2) for N inserts.
 * The custom versions below mutate the radix tree N times under one mutex
 * and call poptrie_rebuild() exactly once at the end, turning a 1M-route
 * load from ~1M rebuilds into 1.
 *
 * The signature here matches the modern in-tree generic_map_update_batch /
 * generic_map_delete_batch prototypes (kernel 6.x).  We slot directly into
 * .map_update_batch / .map_delete_batch in trie_map_ops.
 * ------------------------------------------------------------------------- */

static int trie_update_batch(struct bpf_map *map, struct file *map_file,
			     ...
			     union bpf_attr __user *uattr)
{
	...
	u32 ksize = map->key_size, vsize = map->value_size;
	u32 count = attr->batch.count, done = 0;
	u64 flags = attr->batch.elem_flags;
	struct deferred_val *defer_head = NULL;
	void *kbuf, *vbuf;
	int ret = 0;
	...
	kbuf = kmalloc(ksize, GFP_USER | __GFP_NOWARN);
	vbuf = kmalloc(vsize, GFP_USER | __GFP_NOWARN);
	if (!kbuf || !vbuf) {
		ret = -ENOMEM;
		goto out;
	}
	...
	for (done = 0; done < count; done++) {
		struct bpf_lpm_trie_key_u8 *key = kbuf;
		void *val;
		void *old_val = NULL;
		u32 addr, plen, mask;

		if (copy_from_user(kbuf, ukeys + done * ksize, ksize) ||
		    copy_from_user(vbuf, uvalues + done * vsize, vsize)) {
			ret = -EFAULT;
			break;
		}
		if (key->prefixlen > 32) {
		...
		val = bpf_map_kmalloc_node(map, vsize,
					   GFP_KERNEL | __GFP_NOWARN,
					   map->numa_node);
		if (!val) {
			ret = -ENOMEM;
		...
		memcpy(val, vbuf, vsize);

		ret = __trie_update_locked(trie, addr, plen, val, flags,
					   &old_val);
		if (ret) {
			kfree(val);
			break;
		...
		if (old_val) {
			struct deferred_val *d = defer_wrap(old_val);
	...
	if (done > 0) {
		int rret = poptrie_rebuild(trie, defer_head);
	...
	/* If we never reached rebuild, drop any collected defers safely:
	 * the live snapshot is unchanged so any old_vals we displaced were
	 * either inserted earlier in this same batch (live snap doesn't ref
	 * them) or — for the very first iteration — already protected by
	 * having the live snap still in place; the safe path is to skip
	 * freeing them and let snapshot teardown handle it.  In practice
	 * done > 0 implies rebuild ran. */
	if (copy_to_user(&uattr->batch.count, &done, sizeof(done)) && !ret)
		ret = -EFAULT;
out:
	kfree(kbuf);
	kfree(vbuf);
...
static int trie_delete_batch(struct bpf_map *map,
			     const union bpf_attr *attr,
			     union bpf_attr __user *uattr)
{
	struct lpm_trie *trie = container_of(map, struct lpm_trie, map);
	void __user *ukeys = u64_to_user_ptr(attr->batch.keys);
	u32 ksize = map->key_size;
	u32 count = attr->batch.count, done = 0;
	void *kbuf;
	struct deferred_val *defer_head = NULL;
	int ret = 0;
...
const struct bpf_map_ops trie_map_ops = {
	...
	.map_update_batch  = trie_update_batch,
	.map_delete_batch  = trie_delete_batch,
